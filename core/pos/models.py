...
class Sale(models.Model):
    ...
    def get_cart_total(self):
        saleitems = self.saledetail_set.all()
        total = sum([item.get_total for item in saleitems])
        return total

    @property
    def get_cart_items(self):
        saleitems = self.saledetail_set.all()
        total = sum([item.cant for item in saleitems])
        return total
...
class SaleDetail(models.Model):
    ...
    @property
    def get_total(self):
        total = self.product.get_price_discount * self.cant
        return total
